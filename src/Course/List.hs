...
map ::
  ...
  -> List a
  -> List b
map f = foldRight (\x cur -> f x :. cur) Nil
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f = foldRight (\x cur -> (f x) ++ cur) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight g (Full Nil)
  where g = M.liftM2 (:.)
